...
runcmd:
  ...
  - helm upgrade --kubeconfig /etc/rancher/k3s/k3s.yaml --install demo-argo-cd argo/argo-cd --version ${ARGOCD_VERSION} -n ${ARGOCD_NAMESPACE}
...
#cat ${AZ_SCRIPTS_OUTPUT_PATH}/scriptoutput-v2.json | jq -r '.cloudInitFileAsBase64' | base64 -d
